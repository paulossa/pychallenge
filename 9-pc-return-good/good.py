...
def main():
    ...
    # print "Keyssssss", photo.keys()
    # new = PhotoImage(photo.mode, photo.size)
    #
    ...
    for elt in (first, second):
        for x, y in elt:
            if lastPoint == "":
                photo.put("{yellow}", (x, y))
            else:
                canvas.create_line(lastPoint[0], lastPoint[1], x, y)
            lastPoint = (x, y)

    canvas.create_text(135, 180, text="BULL")
